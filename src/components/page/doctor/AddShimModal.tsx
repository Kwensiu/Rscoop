    ...
    return (
        <dialog class="modal modal-open backdrop-blur-sm" onClick={props.onClose}>
            <div class="modal-box" onClick={stopPropagation}>
                ...
                <h3 class="font-bold text-lg mb-4">{t('doctor.add_shim_modal.title')}</h3>

                <div class="form-control w-full">
                    <label class="label"><span class="label-text">{t('doctor.add_shim_modal.shim_name')}</span></label>
                 ...
                 <div class="form-control w-full">
                    <label class="label"><span class="label-text">{t('doctor.add_shim_modal.command_path')}</span></label>
                 ...
                 <div class="form-control w-full">
                    <label class="label"><span class="label-text">{t('doctor.add_shim_modal.arguments')}</span></label>
                    <input type="text" placeholder={t('doctor.add_shim_modal.arguments_placeholder')} class="input input-bordered w-full"
                        value={args()} onInput={(e) => setArgs(e.currentTarget.value)} />
                     <label class="label"><span class="label-text-alt">{t('doctor.add_shim_modal.arguments_help')}</span></label>
                </div>
                
                 <div class="form-control">
                    <label class="label cursor-pointer">
                        <span class="label-text">{t('doctor.add_shim_modal.global_shim')}</span>
                ...
                <div class="modal-action">
                    <button class="btn btn-primary" onClick={handleAdd} disabled={!name() || !path() || props.isOperationRunning}>
                        <Plus class="w-4 h-4" /> {t('doctor.add_shim_modal.add_shim')}
                    </button>
                </div>
            </div>
        </dialog>
